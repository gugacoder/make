...
namespace Do.PlugIns.VisualStudio
{
  ...
  class BuildCommand
  {
    public bool Exec(BuildCommandOptions options)
    {
      ...
      // Constrói o script de desinstalação.
      MakeUninstallBatches();
    ...
    /// <summary>
    /// Cria script de desinstalação para cada pacote MSI encontrado.
    /// </summary>
    private void MakeUninstallBatches()
    {
      var dir = @"Dist\Setup";
      ...
      var hasSetup = Directory.EnumerateFiles(dir, "*.msi").Any();
      if (hasSetup)
      {
        var msiCommandOptions = new MsiCommandOptions();
        msiCommandOptions.Operands = new OptArgArray();
        msiCommandOptions.Operands.Items = new[] {
          "make-uninstall",
          $@"{dir}\*.msi"
        };
        msiCommandOptions.OutputDir = dir;

        var action = new MakeUninstallAction();
        action.Exec(msiCommandOptions);
      }
    }
